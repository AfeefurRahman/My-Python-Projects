...
def decrypt():
    ...
    if password == "1234":
        ...
        # 🔒 Add app icon to the popup window too
        try:
...
def encrypt():
    ...
    if password == "1234":
        ...
        # 🔒 Add app icon to this popup window too
        try:
...
def main_screen():
    global screen, code, text1, image_icon

    screen = Tk()
    screen.geometry("375x398")
    screen.title("Text Encryption Generator")

    # ✅ FIXED: icon path (must be the image file, not folder)
    icon_path = "/Users/afeefur/Desktop/CSE 115/VS code/My-Python-Projects/My-Python-Projects/Text Encryption Project/keys.png"
    try:
        image_icon = PhotoImage(file=icon_path)
        screen.iconphoto(False, image_icon)
    except Exception as e:
        print(f"⚠️ Icon not found ({e}) — skipping icon setup")
